...
function StudentViewCourseDetailsPage() {
  ...
  async function fetchStudentViewCourseDetails() {
    // Skip purchase info check and always allow viewing course details
    ...
    if (studentViewCourseDetails?.pricing === 0) {
      ...
      try {
        ...
        if (response.success) {
          ...
          alert(response.message || "Could not enroll in free course.");
        }
      } catch (error) {
  ...
  const previewIndex =
    studentViewCourseDetails?.curriculum?.findIndex((v) => v.freePreview) ?? -1;
  const tags = studentViewCourseDetails?.tags || [
    "seo",
    "ads",
    "marketing",
    "linkedin",
  ...
  const instructorName =
    studentViewCourseDetails?.instructorName || "Instructor";
      ...
      <div className="md:col-span-2">
        ...
        <div className="aspect-video rounded overflow-hidden mb-4">
          <VideoPlayer
            url={
              previewIndex !== -1
                ? studentViewCourseDetails?.curriculum[previewIndex]?.videoUrl
                : ""
            }
            width="100%"
            height="100%"
        ...
        <Card className="p-4">
          ...
          <ul className="space-y-1 text-sm text-gray-600">
            <li className="flex items-center gap-2">
              <Globe size={14} /> Language: {language}
            </li>
            <li>
              Lectures:{" "}
              {studentViewCourseDetails?.totalLectures ||
                studentViewCourseDetails?.curriculum?.length ||
                0}
            </li>
